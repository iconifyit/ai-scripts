...
var nyt_png_exporter = {
    ...
    load_prefs: function() {
		...
		if ( this.png_info_lyr.textFrames.length != 1 ) {
			...
			// theTargetFolder = this.base_path;
    ...
    show_dialog: function() {
		...
		var formatList = transPnl.add('dropdownlist', undefined, [ 'PNG 8', 'PNG 24', 'PDF', 'JPG', 'EPS', 'SVG' ]);
		...
		for (var i = starting_artboard; i < num_artboards; i++ ) {
			...
			if ( this.export_code == 'layers' || ! ( artboardName.match(  /^artboard/i ) || artboardName.match( /^\-/ ) )) {
				...
				if ( this.export_code == 'artboards' ) {
					...
					var nextName = artboardName.replace (" ", "-");
						nextName = nextName.replace("(Line)", "line");
						nextName = nextName.toLowerCase();
					var base_filename = this.base_path + "/" + this.prefix + nextName + this.suffix
					...
					if ( this.format.match( /^PNG/ )) {                
						...
						options.artboardRange = (i+1).toString();
						docRef.saveAs( destFile, options )
					} else if ( this.format.match( /^SVG/ )) {
						try {
				
							var destFile = new File(base_filename + '.svg'); 
